...
export function EmployeeSalesRecordsCards() {
  const { data: salesRecords, isLoading, isError, error } = useEmployeeSalesRecords();
  ...
  if (!salesRecords || salesRecords.length === 0) {
    return (
      <p className="text-gray-600 text-center py-4">No sales records found.</p>
    );
  }

  return (
    <div className="flex-col justify-center p-3">
      <h2 className="text-2xl font-bold m-4">Your Sales Records</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {salesRecords.map((record) => (
          <Link
            key={record.salesRecordId}
            to="/employee/sales/$name"
            params={{ name: record.salesRecordId }}
            className="block" // Make the link a block element to wrap the card
          >
            <Card className="bg-white border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200 cursor-pointer h-full">
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-semibold text-gray-800">
                  Sale to {record.customerName}
                </CardTitle>
